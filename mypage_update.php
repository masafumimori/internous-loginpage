<?php
mb_internal_encoding("utf8");

// セッションスタート
session_start();

if (isset($_POST['update'])){
  // DB接続・try catch文
  try {
    $pdo = new PDO ("mysql:dbname=masafumi;host=localhost;", "root", "");
  }catch (PDOException $e){
    die("<p>申し訳ございません。現在サーバーが混み合っており一時的にアクセスができません。
    <br>
    しばらくたってから再度ログインしてください。</p>
    <a href='http://localhost/login_mypage/login.php'>ログイン画面へ</a>"
    );
  }
  
  // Preparedステートメント（Update)でSQLをセット　Bindvalueでパラメータをセット（変化しない$_SESION['id']で何処に挿入するかを判断）
  $stmt = $pdo -> prepare("update login_mypage set 
    name = ?, mail = ?, password = ?, comments = ?
    where id = ?");
  
  // 順番に注意&$_POST
  $stmt -> bindValue(1, $_POST['name']);
  $stmt -> bindValue(2, $_POST['mail']);
  $stmt -> bindValue(3, $_POST['password']);
  $stmt -> bindValue(4, $_POST['comments']);
  $stmt -> bindValue(5, $_SESSION['id']);
  
  // Executeでクエリを実行
  $stmt -> execute();
  
  // Preparedステートメント（更新された情報をDBからSelect文でSQLをセット）BindValueでパラメータをセット($_POST)
  $stmt = $pdo -> prepare("select * from login_mypage where mail = ? and password = ?");
  $stmt -> bindValue(1, $_POST['mail']);
  $stmt -> bindValue(2, $_POST['password']);

  // Executeでクエリを実行
  $stmt -> execute();
  
  // データベースを切断
  $pdo = NULL;
  
  // Fetch/while文でデータを取得し、Sessionに代入
  while ($row = $stmt -> fetch()){
    $_SESSION['id'] = $row['id'];
    $_SESSION['name'] = $row['name'];
    $_SESSION['mail'] = $row['mail'];
    $_SESSION['password'] = $row['password'];
    $_SESSION['picture'] = $row['picture'];
    $_SESSION['comments'] = $row['comments'];
  };

  // Mypage.phpへリダイレクト
  header("Location:http://localhost/login_mypage/mypage.php");
}else {
  header("Location:http://localhost/login_mypage/login.php");
}
